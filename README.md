...
- **No reconnect/resume.** If someone's page reloads mid-game, they rejoin as a
  brand-new player — there's no login/session persistence. Fine for a casual game,
  but I can add reconnect tokens if it becomes annoying.
- **Host has no fallback.** If the host disconnects, nobody currently gets
  promoted to host. Easy to add if you want it.
