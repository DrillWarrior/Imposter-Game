(function () {
  ...
  let state = {
    screen: 'landing',   // landing | joinForm | room
    name: '',
    codeInput: '',
    myId: '',
    code: '',
  ...
  // ---------- SOCKET EVENTS ----------
  socket.on('created', ({ code, playerId }) => {
  ...
  window.__imp_leaveCase = function () { location.reload(); };
    ...
    if (room.status === 'lobby') {
      const playerRows = room.players.map(p => `
        <div class="player-row">
          ...
          ${p.id === room.hostId ? '<span class="player-host-tag">Host</span>' : ''}
        </div>`).join('');
